class Ad < ActiveRecord::Base
  ...
  def self.find_ads_by_parts(part_ids)
    ...
    joins(:parts).select("ads.*").group("ads.id, ads.description, ads.created_at, ads.image_url, ads.price, ads.available, ads.title, ads.color, ads.material, ads.size, ads.condition").where("parts.id in (?)", part_ids).having("count(*)=#{part_ids.size}")
